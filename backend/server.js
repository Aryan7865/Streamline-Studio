...
app.get("/auth/google/callback", async (req, res) => {
  ...
  try {
    ...
    res.redirect(
      `http://localhost:5173/upload-result?success=false&error=${encodeURIComponent(
        error.message
      )}`
    );
  }
...
app.get("/publish/:videoId", async (req, res) => {
  ...
  try {
    ...
    res.redirect(
      `http://localhost:5173/upload-result?success=true&videoId=${videoId}`
    );
  } catch (error) {
    console.error(
...
async function storeTokensInDatabase(userId, tokens) {
  try {
    ...
    const tokenExpiryDate = new Date(
      Date.now() + (tokens.expires_in ? tokens.expires_in * 1000 : 0)
    );
