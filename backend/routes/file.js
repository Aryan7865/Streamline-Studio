...
router.get("/raw", auth, async (req, res) => {
  try {
    ...
    if (user.role === "YouTuber") {
      const rawVideos = await RawFile.find({ uploader: user._id }).sort({
        uploadedAt: -1,
      ...
      const rawVideos = await RawFile.find({ uploader: assUser._id }).sort({
        uploadedAt: -1,
      }); // Sort by newest first
...
router.get("/edited", auth, async (req, res) => {
  try {
    ...
    if (user.role === "Editor") {
      const editedVideos = await EditedFile.find({ uploader: user._id }).sort({
        uploadedAt: -1,
      }); // Sort by newest first
      ...
      const assUser = await User.findOne({ email: associatedUserEmail });
      const editedVideos = await EditedFile.find({
        uploader: assUser._id,
...
router.post("/save-raw", auth, async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({
        message: "Raw video info saved successfully",
        video: newRawFile,
      });
  } catch (error) {
...
router.post("/save-edited", auth, async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({
        message: "Edited video info saved successfully",
        video: newEditedFile,
      });
  } catch (error) {
